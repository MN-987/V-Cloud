	...
	if [[ "$network" != "localhost" ]]; then
		if (( $2 == 1 )); then
			...
			while [[ $smart_contract_address == "" ]]; do
				...
				warn "Please 98 check your node is up and running and try again"
				info "Retrying in 5 seconds..."
				sleep 5s
				deploy_to_network
			done
			info "A new smart contract address: $(decorize_address) is cached"
			sed -i "s/SMART_CONTRACT_ADDRESS_SEPOLIA.*$/SMART_CONTRACT_ADDRESS_SEPOLIA=$smart_contract_address/" ./.env
		else
			smart_contract_address=$(grep "SMART_CONTRACT_ADDRESS_SEPOLIA" ./.env | cut -d "=" -f 2)
		...
		while [[ $smart_contract_address == "" ]]; do
			error "Unable to deploy the smart contracts to $network network"
			warn "Please 117 check your node is up and running and try again"
			info "Retrying in 5 seconds..."
			sleep 5s
			deploy_to_network
		done
